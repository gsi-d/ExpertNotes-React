      ...
      <Dialog.Root>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="z-10 fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
            <Dialog.Close onClick={handleClickClose} className="absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100">
              <X className="size-5"/>
            </Dialog.Close>
            <form onSubmit={(e) => handleSaveNote(e)} className="flex flex-1 flex-col">
              ...
              :
                <button disabled={value == ""} onClick={(e) => handleSaveNote(e)} type="button" className="w-full bg-lime-400 py-4 text-center text-sm text-lime-950 outline-none font-bold hover:bg-lime-500">
                  Salvar nota
                </button>
              }
